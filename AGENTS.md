...
10. **独立审查 SOP**（2026-08-29 新增，源于全库 24 书批次审查实战——Traitors' Nest 4 项 + Natural Selection 10+ 项缺陷；审查方与执行方分离场景的标准化流程）：
    ...
    **⚠️ 审查触发方式（2026-09-18 用户拍板）：五步审查由用户主动发起——执行方不自动执行全书级五步审查。**
    ...
    - **同会话审查的执行要求（防走形式）**：① 门禁**全部重跑**，禁止采信自己此前贴过的数字；② 逐章归属、crossref、总览三层用**与写作时不同的检查路径**（换工具/换口径/全量 grep，而非复述原分析）；③ d 步语义二审**必须派不带写作上下文的子代理**逐对核对（附 1–2 个本库真实失败案例 + 防幻觉条款）；④ 不得因"是我写的"而在报警环节自我豁免——本项目同会话五步审查多次抓出自身缺陷（Tinder Box 3 章跨章错植 + 6 条金句虚构、Ken Liu 接手批次 18 条引语窜章等）。
    - **局限如实标注**：同会话审查对"写作时的系统性误判"（如全书统一误读某设定、人物关系系统性错写）检出率低于异实例审查——此类维度须在结论里显式写"同会话审查的已知盲区"，建议存疑或重大批次时由用户另行指派异实例复核（建议而非强制）。
    ...
    **完成报告硬要求（执行方交付审查时必填，否则审查方有权拒收）**：
    ...
    d. **语义二审**：引语↔分析逐对核对。**子代理委派必须附 1-2 个本库真实失败案例 + 防幻觉条款**（定义与实证见第 9 条 f，此处不重复）；**子代理额度耗尽时主会话自执行不可省**——证据：NS 137 块三件代理被 Token Plan 拦下，主会话逐对核对仍可完成全部。
...
- **协作板更新节奏**：批次进行中不逐批更新 COLLABORATION.md；**全书完成（五步审查之前）统一更新一次**——协作板发本书唯一完工通报 + 当日工作日志记本书条目，均附全部 commit 编号；**独立五步审查之后，把审查结论/整改情况就地追加进同一条板消息与同一篇日志的本书条目内（不新开条目）**；push 前再就地更新最终状态
