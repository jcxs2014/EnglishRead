...
10. **独立审查 SOP**（2026-08-29 新增，源于全库 24 书批次审查实战——Traitors' Nest 4 项 + Natural Selection 10+ 项缺陷；审查方与执行方分离场景的标准化流程）：
    ...
    **⚠️ 强制条款（2026-09-18 用户拍板）**：**五步审查是完工的组成部分，不是可选项。**书籍"完工"的定义 = 正文全部章节 + 总览三篇（短篇合集除外）+ 五步审查通过；四件套全绿只算半个完工。执行方在全书写完后**必须立即自行启动五步审查，不得等用户催办**；**优先审执分离**（委派另一实例执行审查），无并行实例时执行方自审且 a–e 五步一步不得省略（d 步可分子代理并行，须附 1–2 个本库真实失败案例 + 防幻觉条款）。审查发现缺陷 → 修复 → 重跑对应门禁 → 复验通过后方可发完工通报。**完工通报必须附五步审查结果**（各步结论 + 三件套原始数字 + d 步缺陷清单与修复 commit + e 步总览 MISS 数）；未做五步审查的交付按"未完工"处理，用户或审查方有权要求补做后才放行。**实证依据**：本项目多本书在四件套全绿下藏有语义层缺陷（Tinder Box 3 章跨章错植 + 6 条金句虚构、Daggerbound 71 处、Adrift 33 处、Natural Selection 10+ 处），五步审查是目前唯一能系统性抓出该层的门禁。
