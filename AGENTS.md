...
10. **独立审查 SOP**（2026-08-29 新增，源于全库 24 书批次审查实战——Traitors' Nest 4 项 + Natural Selection 10+ 项缺陷；审查方与执行方分离场景的标准化流程）：
    ...
    **⚠️ 强制条款（2026-09-18 用户拍板）：两道审查，独立审查是完工的最后一关。**书籍"完工"的定义 = 正文全部章节 + 总览三篇（短篇合集除外）+ **独立审查通过**；四件套全绿只算半个完工。

    - **第 1 道｜自审（执行方）**：全书写完后**必须立即自行执行五步法，不得等用户催办**；a–e 一步不得省略（d 步可分子代理并行，须附 1–2 个本库真实失败案例 + 防幻觉条款）。
    - **第 2 道｜独立审查（强制，抓漏主力）**：自审有结构性盲区——同一会话共享写作时的假设与记忆，自审查不出自己的系统性误判（本项目实证：多本书自审通过后仍被独立审查抓出缺陷清单）。因此**独立审查不得由精读会话自身完成**，必须由「另一实例 / 同机另一独立会话 / 用户指派的新会话」执行；**同一会话内的自审与子代理一律不计入独立审查**。
    - **状态口径**：`完工待审` →（独立审查）→ `审查通过` = 完工释放；`审查通过` 前不得自称"完工"，不得发完工通报（可发"待审通报"）。
    - **执行方收尾三动作（强制）**：① 按 `docs/独立审查任务书模板.md` 生成自包含审查任务书（放 `docs/reviews/<slug>.md`）；② 在 COLLABORATION.md 发「待独立审查」通报（书目录 + 自审数字 + 任务书路径）；③ 向用户如实报告"待独立审查"，并给出审查方一键启动指令。
    - **独立审查方义务**：重跑门禁（不信自审数字）→ 五步法 a–e → 四类高发坑位 → d 步带反例与防幻觉条款；输出缺陷清单（附文件+行号+证据）→ 执行方修复 → 重跑 → 复验；审查方在协作板回帖给出结论（通过 / 整改后通过 / 不通过）。
    - **无人可审时**：状态保留 `完工待审`，等用户指派独立审查方（或用户在另一 IDE 里启动"审查方"模板）；**用户明确决定暂缓/不审时，也必须在通报与记忆中如实标注"独立审查未做（用户决定）"——禁止静默略过**。
    - **完工通报必附**：自审五步结果 + 三件套原始数字 + **独立审查结论（审查方身份 / 报告位置 / 修复 commit 清单）**；缺独立审查记录的交付按"未完工"处理，审计时一律列为待办。
    - **实证依据**：多本书在四件套全绿下藏语义层缺陷（Tinder Box 3 章跨章错植 + 6 条金句虚构、Daggerbound 71 处、Adrift 33 处、Natural Selection 10+ 处），且自审通过的批次仍被独立审查抓出大量问题（Natural Selection 总览层 4 类、Room 说话人误归 37%）——独立审查是目前唯一被反复证明有效的最后一道门禁。
