...
| **推理 / 悬疑 / 奇幻小说（长篇）** | 逐章精读（精简格式）+ 3 篇总览 | 有推理线 / 线索 / 嫌疑人结构，需逐句 |
| **随笔集 / 书评集** | 逐篇精读（1 H1 + 4 H2）+ 3 篇总览 | 每篇独立，如 BTSML 模式 |
| **短篇合集（科幻/悬疑/恐怖/奇幻）** | 逐篇精读（10处 + 五子项 + 三档词汇 + 一句话总结） | 每篇独立故事，如 *Alfred Hitchcock* / *Tales of Terror*；**短篇合集是唯一不要求总览三篇的体裁** |
| **非虚构论述（科普/社科/商业）** | 逐章精读 + 论证结构分析 + 3 篇总览 | 核心价值在论证链，纯语言精读会丢失，如 *Inside the Box* |
| **诗歌回忆录（memoir in verse）** | 逐章精读 + 诗歌技法专项 + 3 篇总览 | 自由诗，按"随笔集逐篇精读"框架适配，新增"诗歌技法专项"章节分析跨行连续/括号自反/通感联觉/自造词等，如 *Ten Bridges I've Burnt* |

**⚠️ 总览三篇强制规则（2026-09-18 用户拍板）**：**除短篇合集外，所有书籍完工时都必须创建总览三篇**（`概述.md` / `金句精选.md` / `情感节点.md`）——精简格式（推理/悬疑/奇幻）、随笔集、非虚构论述、诗歌回忆录一律适用，"精简格式不含总览"不再是可选项。**历史豁免一次性生效**：2026-09-18 前已完工并通报"无总览三篇"的书籍（asmodeus / bury-your-dead / meet-cute-magic 等）经用户拍板不补做；本规则自 2026-09-18 起对新书与在制书（尚未完工者）生效。
...
全书完成后创建 3 篇总览文件（**除短篇合集外所有体裁通用，2026-09-18 起强制**）：
...
5. **梗概与词汇同受原文约束**（check_vocab / check_entities 已机械化，此条定义 FAIL 的处理规范）：
   ...
   - **FAIL 的 A/B 裁决**：check_vocab 报 FAIL 时，先以 epub 展平全文终极裁决——epub 也查无 = A 类真虚构，换文中真实词；epub 有而 text/ 缺 = B 类语料缺失，优先改用文中真实词形（如 intoxicated 替代 intoxication），或重跑 `extract_chapters.py` 修复提取覆盖。禁止不裁决直接删词条了事；**flat 整串查无 ≠ A 类——判 A 前必须拆 fragment 分段取证**（各片段分别 flat 查 + 前后文 ~150 字符窗口 grep）：She Haunts 批次 9 条 text/epub flat 查无中 7 条经字节级取证全部是跨标签拼接（两半逐字都在原文、中间叙述标签被换成句号），整串 flat 口径天然对拼接假阴为"查无"，直接判 A 类会误杀真引语（2026-09-22 实证）；
...
8. **第一遍内联验证流程**（2026-08-29 新增，针对"生成后检查"而非"边生成边验证"的根因漏洞）：
   ...
   **每章精读完成的强制 Gate（不等 commit 前才跑）**：
   ...
   d. **章末核验**：每章写完立即跑 `verify_quotes` 和 `check_vocab`，**FAIL=0 才推进下一章**。不等到批次 commit 前才跑：
      ...
      ```
   e. **关键词锚定自建检查器（≥20 章推荐）**：规则 9b 靠人工核在大批次必然漏——自建锚定检查器（关键词英文词须命中引语或"为什么这样写"文本，允许词形变化，剔除 stop words）。Memories Like Fangs 实战一次扫 237 块抓出 59+ 处违规
...
9. **引语修复同步规则**（2026-08-29 新增，源于全库 24 书语义对应审计：~20 处"引语已换、分析停旧句"在 verify_quotes 全绿下漏网）：
   ...
   c. **结构完整**：拆分/删除引语块后必须重排编号保持连续；每块必须"引语行 + 全部分析子项"齐全，孤儿块（有分析无引语）与重复块（同引语出现两次）均视为缺陷。**删除跨章错植块后若该章剩余块数低于 3–8 配额下限，必须用本章真实引语回填**（She Haunts ch10 删 4 剩 1 实证，2026-09-22）。
   ...
   g. **正则替换禁止 re.S 跨块**：修复脚本必须行级作用域，禁止 `re.S` 跨块 sub 操作——Memories Like Fangs 修复脚本 re.S 跨块 sub 一次损毁全书 116 处结构（引语行 `> ` 前缀丢失 + `**为什么这样写**` 标头被误吞），git checkout HEAD 回滚重来
   h. **总览三篇只能行级 edit，禁止整文件 write/覆盖**（2026-09-22 新增）：三篇总览文件名近似、结构同构，整文件写入极易张冠李戴——实证：Impossible Garden `abfe084f` 把修过的金句精选**整文件写进 `00_情感节点.md`**（H1 都变成 `# 金句精选`），16 个节点全部丢失，修复报告却称"完成"；恢复用 `git show <覆盖前基线commit>:<路径>`。**配套机检（进第 10 条 c 步）**：`grep -m1 '^# ' 00_*.md`——H1 必须与文件语义一致（金句精选 H1 不得出现在情感节点文件里），一行 grep 即可抓住此类事故。
...
10. **独立审查 SOP**（2026-08-29 新增，源于全库 24 书批次审查实战——Traitors' Nest 4 项 + Natural Selection 10+ 项缺陷；审查方与执行方分离场景的标准化流程）：
    ...
    **⚠️ 审查触发方式（2026-09-18 用户拍板）：五步审查由用户主动发起——执行方不自动执行全书级五步审查。**

    - **执行方分内工作（保留）**：每章/每批的内联 Gate（第 8 条）+ 提交前四件套自审 + 总览三篇门禁——这些照旧，是完工的必要条件。
    - **五步审查（本节 a–e）不在执行方自动流程内**：全书写完后**不得自行启动**全书级五步审查，也**无需**生成审查任务书或发"待审通报"；完工通报只报告四件套数字与总览门禁结果，注明"五步审查未做（待用户发起）"。
    - **触发方式**：用户发话（"独立进行五步审查"/"审查这本书"）时启动，按本节 a–e 五步流程执行。
      **执行方由用户指定**——用户**在同一会话内要求时，本实例直接执行**（这是用户的常规习惯，不得因"同会话"而降级或推辞）；用户指派另一实例/另一会话时，由被指派方执行，执行方按下方"完成报告硬要求"提供材料并配合修复整改。
    - **同会话审查的执行要求（防走形式）**：① 门禁**全部重跑**，禁止采信自己此前贴过的数字；② 逐章归属、crossref、总览三层用**与写作时不同的检查路径**（换工具/换口径/全量 grep，而非复述原分析）；③ d 步语义二审由主会话或派子代理执行均可，以完成全部引语↔分析逐对核对为准（附 1–2 个本库真实失败案例 + 防幻觉条款）；④ 不得因"是我写的"而在报警环节自我豁免——本项目同会话五步审查多次抓出自身缺陷（Tinder Box 3 章跨章错植 + 6 条金句虚构、Ken Liu 接手批次 18 条引语窜章等）。
    - **局限如实标注条款的适用范围**：本条仅适用于"执行方自行发起自审（无用户指令）"的场景。**用户在同一会话内主动要求执行五步审查时，该条款不适用——此时 a–e 五步须完整执行，不得以"同会话局限"为由跳过任何步骤或降低标准**。"局限"指审查报告的结论部分须如实写明同会话审查的已知盲区（如全书统一系统性误判），供用户判断是否需要另行指派异实例复核——这是结论写作的要求，不是减少审查步骤的依据。
    - **历史依据**：五步审查是抓漏主力（多本书在四件套全绿下藏语义层缺陷——Tinder Box 3 章跨章错植 + 6 条金句虚构、Daggerbound 71 处、Adrift 33 处、Natural Selection 10+ 处）——正因如此，它**值得每次认真执行**，但由用户掌握批次节奏与指派，避免执行方自行发起的"自审式审查"稀释其独立性。
    ...
    **完成报告硬要求（执行方交付审查时必填，否则审查方有权拒收）**：
    ...
    a. **三件套重跑（不信报告数字）**：`verify_quotes` / `check_vocab` / `check_entities` 全部从本机重跑——报告里"X/X ✅"必须当场复验，禁止直接采纳。**实证**：NS 报告 101/101 实为 108/109 含 1 FAIL；TN 报告 44/44 包含 1 处跨章错植未被报。**"修复报告"与完工报告同权，逐项现场复验**（2026-09-22 实证：三本书修复报告 2/3 与现场不符——声称"金句㉑已修"实为修了另一个文件、"修复情感节点"实为整文件覆盖、计数口径 168 vs 实测 169）。
...
- "测试我" = 设计练习不直接给答案
- "不要翻译" = 全英文用简单英文解释英文
- **"独立进行五步审查" = 对本会话当前书籍完整执行第 10 条五步流程（a–e 不得省略，输出缺陷清单并完成整改），用户在本会话内要求即为合法常规路径，不得因"同会话"而降级或推辞——但须如实标注同会话审查的已知局限（见第 10 条）**
