---
状态: 未读
modified: "2026-09-25"
source_text: ch50_49_be_wary_when_things_get_off_to_a_grea.txt
---
# 50. Be Wary When Things Get Off to a Great Start（事情开局顺利时要警惕）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 49
- **作者**：Rolf Dobelli
- **章节定位**：关于新手运气
- **字符数**：4,268
- **一句话主旨**：短期成功可能来自运气而非能力，必须用长期竞争记录和反证方法区分两者。

## 论证结构

- **核心论点**：新手运气会把偶然结果误认为稳定能力，诱使人加码并最终暴露。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 赌场新手 | 案例 | 早期好运会推高赌注 |
| 公司连续收购 | 商业案例 | 成功整合让人误判能力 |
| 互联网股票与住房泡沫 | 金融案例 | 整体上升趋势制造个人能力幻觉 |
| 长期高手与参与者数量 | 推理 | 竞争者越多，偶然领先越常见 |
| 作者投稿十家出版社 | 个人实验 | 多次反驳才能检验成功理论 |

- **论证脉络**：前章联结偏误 → 新手运气 → 企业、投资和房地产案例 → 判断能力的方法 → 主动寻找反证。
- **可质疑处**：作者把“长期相对领先”视为能力证据，但样本选择、技能变化和运气分布仍需更严格检验。

## 选择性精读

### ① 新手运气

> **原句 1:** "Convinced of their above-average skills, these amateurs increase the stakes."

**中文理解**：这些业余者确信自己技能高于平均水平，于是提高赌注。

**句子结构**：Convinced of... 为过去分词短语作状语；these amateurs 为主语；increase the stakes 为谓语。

**关键词**：above-average, amateurs, increase the stakes

**表达方式**：用信念—行为链说明运气如何转化为风险。

**为什么这样写**：短期胜利先改变自我认知，再改变实际下注规模。

### ② 连续收购

> **原句 2:** "The acquisitions prove a success, and the directors believe they have real skill for acquisitions."

**中文理解**：这些收购取得成功，董事们因此相信自己具有真正的收购能力。

**句子结构**：The acquisitions 为主语；prove a success 为谓语；and 后的 directors believe... 为并列分句。

**关键词**：acquisitions, directors, real skill

**表达方式**：把企业结果与管理者能力直接连接。

**为什么这样写**：连续成功会制造归因幻觉，而没有验证市场条件的作用。

### ③ 泡沫中的“能力”

> **原句 3:** "These investors overlooked one tiny detail: Their amazing profits at the time had nothing to do with their stock-picking abilities."

**中文理解**：这些投资者忽略了一个小细节：当时的惊人利润与选股能力毫无关系。

**句子结构**：investors 为主语；overlooked 为谓语；冒号后的独立句解释被忽略的细节。

**关键词**：overlooked, amazing profits, stock-picking abilities

**表达方式**：用冒号把关键遗漏直接摊开。

**为什么这样写**：它把市场上涨造成的共同收益与个人选择能力明确拆开。

### ④ 长期检验

> **原句 4:** "If you are much better than others over a long period of time, you can be fairly sure that talent plays a part."

**中文理解**：如果你长期明显优于他人，可以相当有把握地认为才华起了作用。

**句子结构**：If 引导条件状语从句；主句 you can be fairly sure；that talent plays a part 为同位语从句。

**关键词**：long period, fairly sure, talent, plays a part

**表达方式**：用保留语 fairly sure 避免绝对化断言。

**为什么这样写**：能力判断需要时间尺度，但作者仍承认无法达到百分之百确定。

### ⑤ 主动反证

> **原句 5:** "Try to disprove them."

**中文理解**：尝试证伪这些理论。

**句子结构**：祈使句 Try to disprove them；disprove 为动词不定式。

**关键词**：Try, disprove, theories

**表达方式**：用简短的祈使句把科学方法转成行动指令。

**为什么这样写**：面对成功解释，单纯寻找支持证据会巩固幻觉；必须主动寻找反例。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| beginner’s luck | 新手运气 | "beginner’s luck" |
| acquisition | 收购 | "company A buys smaller companies" |
| integration | 整合 | "The integration is a disaster." |
| huge | 巨大的 | "a much larger company, E." |
| disprove | 证伪 | "Try to disprove them." |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| stake | 赌注 | "increase the stakes" |
| profit | 利润 | "amazing profits" |
| bubble | 泡沫 | "U.S. housing boom" |
| debt | 债务 | "mountains of dot-com debt" |
| unsellable | 卖不掉的 | "unsellable properties" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| skill | "their stock-picking abilities" |
| investor | "many investors" |
| market | "The market was simply on an upward spiral." |
| history | "history has no shortage" |

## 一句话总结

开始顺利时最该做的事不是加大投入，而是扩大样本、延长时间并主动寻找反证。
