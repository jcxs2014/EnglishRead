---
状态: 未读
modified: "2026-09-25"
source_text: ch60_59_if_you_have_an_enemy_give_him_informa.txt
---
# 60. If You Have an Enemy Give Him Information（如果你有敌人，就把信息给他）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 59
- **作者**：Rolf Dobelli
- **章节定位**：关于信息偏误
- **字符数**：4,313
- **一句话主旨**：更多信息并不自动改善决策；如果新增信息不能改变行动，它只会增加时间、金钱和过度自信。

## 论证结构

- **核心论点**：信息价值取决于它是否改变选择，不取决于数量；知识和数据堆积可能制造决策幻觉。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| Borges 1:1 地图 | 思想实验 | 信息完整也可能没有洞察 |
| 酒店搜索经历 | 日常案例 | 大量评论未改善最初选择 |
| Baron 医生诊断题 | 概率实验 | 无关信息不改变治疗选择 |
| 城市人口熟悉度 | 研究案例 | 更多已知信息有时导致错误答案 |
| 金融报告与危机 | 宏观案例 | 大量预测在危机中失效 |

- **论证脉络**：地图极端案例 → 酒店研究 → 医生概率题 → 城市比较 → 金融数据崇拜 → “知识的幻觉”。
- **可质疑处**：信息有时确实改变决策；关键是区分与行动相关的决策相关信息和无关、过时或高噪声资料。

## 选择性精读

### ① 一比一地图

> **原句 1:** "A map with a scale of 1:1, as large as the country itself."

**中文理解**：一张比例尺为 1:1、与国家本身一样大的地图。

**句子结构**：介词短语 with a scale of 1:1 修饰 map；as large as the country itself 为比较结构。

**关键词：** **：map, scale, 1:1, country itself

**表达方式**：用精确比例和荒诞尺寸构成思想实验。

**为什么这样写**：信息最大化并不能保证洞察；完整复制现实可能只让人更拥挤地站在现实里。

### ② 酒店选择
> **原句 2:** "The mountain of additional information did not lead to a better decision."

**中文理解**：堆积如山的新增信息并没有带来更好的决定。

**句子结构**：The mountain of additional information 为主语；did not lead to 为否定谓语；a better decision 为宾语。

**关键词：** **：mountain, additional information, did not lead, better decision

**表达方式**：用 mountain 视觉化信息数量，同时否定其决策价值。

**为什么这样写**：酒店作者把“继续搜索”误当成“提高判断力”，实际上增加了成本。

### ③ 无关诊断信息
> **原句 3:** "The additional information contributes nothing of value to the decision."

**中文理解**：这些额外信息对决定毫无价值。

**句子结构**：The additional information 为主语；contributes nothing of value 为谓语；to the decision 为状语。

**关键词：** **：additional information, contributes nothing, value, decision

**表达方式**：用 nothing of value 明确否定信息的决策相关性。

**为什么这样写**：信息是否存在不是重点，它是否改变最优行动才是。

### ④ 信息更多却猜错
> **原句 4:** "They had more information, and it misled them."

**中文理解**：他们掌握了更多信息，而这些信息误导了他们。

**句子结构**：They 为主语；had 为谓语；and it misled them 为并列分句。

**关键词：** **：more information, misled

**表达方式**：用 and 制造信息量与判断质量的反常关系。

**为什么这样写**：熟悉度会让人把“不熟悉”误当成“更少”，从而破坏概率判断。

### ⑤ 知识的幻觉
> **原句 5:** "The greatest obstacle to discovery is not ignorance—it is the illusion of knowledge."

**中文理解**：发现的最大障碍不是无知，而是知识的幻觉。

**句子结构**：The greatest obstacle 为主语；not ignorance 与 the illusion of knowledge 构成对比，破折号连接两个名词短语。

**关键词：** **：greatest obstacle, discovery, ignorance, illusion of knowledge

**表达方式**：用 not...but... 的强调对比改写格言。

**为什么这样写**：拥有大量数据却不行动，可能比承认无知更危险，因为它会阻止真正探索。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| information bias | 信息偏误 | "Information Bias" |
| cartography | 制图学 | "The science of cartography" |
| superfluous | 多余的 | "Superfluous knowledge is worthless" |
| surplus | 多余的 | "surplus information" |
| diagnostic | 诊断的 | "a diagnostic test" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| insight | 洞察 | "does not provide any insight" |
| symptom | 症状 | "presents symptoms" |
| treatment | 治疗 | "each treatment" |
| economist | 经济学家 | "hundreds of thousands of economists" |
| forecast | 预测 | "countless forecasts" |

### ⭐ 基础

| 词/phrase | 例句（原文） |
|---|---|
| map | "the most detailed of maps" |
| hotel | "Searching for a hotel" |
| city | "Which city has more inhabitants" |
| disease | "suffering from disease A" |
| data | "all the data" |

## 一句话总结

信息不是越多越好；先问新增证据是否会改变行动，如果不会，堆积资料只会把无知伪装成知识。