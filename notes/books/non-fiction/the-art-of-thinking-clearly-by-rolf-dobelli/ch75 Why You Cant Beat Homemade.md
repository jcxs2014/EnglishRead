---
状态: 未读
modified: "2026-09-25"
source_text: ch75_74_why_you_can_t_beat_homemade_not_inven.txt
---
# 75. Why You Cant Beat Homemade（ homemade 方案为何总觉得最好）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 74
- **作者**：Rolf Dobelli
- **章节定位**：关于 NIH 综合症
- **字符数**：4,327
- **一句话主旨**：人们偏爱自己提出的方案；通过跨组评审、交换评价和回看旧想法可以降低这种偏见。

## 论证结构

- **核心论点**：内部自创方案容易获得过高评价，外部方案即使同样优秀也可能被低估；组织需要把提出和评价分开。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 酱汁实验 | 个人故事 | 自制方案的评价并不客观 |
| 保险软件案例 | 商业案例 | 内部系统会压低外部产品价值 |
| 两组分工 | 组织方案 | 评价者与提出者分离可减少偏见 |
| Ariely 读者实验 | 实验研究 | 相同建议也会被评为更适合自己的方案 |
| 环形交叉路口 | 社会案例 | 外部创新可能长期遭到忽视 |

- **论证脉络**：个人失败 → 公司惯性 → 团队分离 → 受控实验 → 社会扩散 → 回顾检验。
- **可质疑处**：外部方案也需要本地知识和适配成本；不能因为 NIH 综合征存在，就一律把他作方案视为更优。

## 选择性精读

### ① 自制酱汁

> **原句 1:** "I was guilty of the not-invented-here syndrome."

**中文理解**：我犯下了 NIH 综合症，也就是“不在本地发明的都不如自制”的错误。

**句子结构**：I 为主语；was guilty of 为系表结构；the syndrome 为介词宾语。

**关键词**：guilty, not-invented-here syndrome, NIH syndrome, self

**表达方式**：用自嘲承认判断错误。

**为什么这样写**：把一次烹调失败变成可迁移的组织认知案例。

### ② 外部方案

> **原句 2:** "Most insurers are convinced that the best solution is what they have crafted themselves in-house."

**中文理解**：大多数保险公司确信，最好的解决方案就是他们自己内部打造的系统。

**句子结构**：Most insurers 为主语；are convinced that... 为宾语从句；what they have crafted... 为表语。

**关键词**：insurers, convinced, best solution, crafted themselves

**表达方式**：用“内部打造”突出自我评价的封闭性。

**为什么这样写**：它把 NIH 从个人偏好扩展为机构购买软件时的系统性失败。

### ③ 团队分工

> **原句 3:** "The first group generates ideas, the second rates them, and vice versa."

**中文理解**：第一组提出想法，第二组评价想法，反之亦然。

**句子结构**：三个并列分句共享主语逻辑；first、second 和 vice versa 形成交换关系。

**关键词**：generates ideas, rates them, vice versa, groups

**表达方式**：用对称结构给出组织流程。

**为什么这样写**：交换生成与评价角色，能降低自己评价自己造成的系统性高估。

### ④ 相同答案

> **原句 4:** "the majority rated their own responses as more important and applicable than the others"

**中文理解**：多数人认为自己的回答比别人更重要、更适用。

**句子结构**：the majority 为主语；rated...as... 为谓语；than the others 为比较结构。

**关键词**：majority, own responses, important, applicable

**表达方式**：用自我优先的评价结果呈现偏见。

**为什么这样写**：当建议被统一限制为相同答案时，差异只能来自评价者的自我偏好。

### ⑤ 回看

> **原句 5:** "We are drunk on our own ideas."

**中文理解**：我们沉醉在自己的想法里。

**句子结构**：We 为主语；are drunk on 为系表短语；our own ideas 为介词宾语。

**关键词**：drunk, own ideas, sober up, hindsight

**表达方式**：用醉酒比喻自负与执着。

**为什么这样写**：它把 NIH 综合症的情绪本质压缩成一句可记忆的警告。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| not-invented-here syndrome | NIH 综合症 | "the not-invented-here syndrome" |
| in-house | 内部的 | "crafted themselves in-house" |
| applicable | 适用的 | "more important and applicable" |
| resistance | 抵制 | "oblivion and resistance" |
| hindsight | 事后视角 | "in hindsight" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| sauce | 酱汁 | "the sauce" |
| recipe | 食谱 | "a new recipe" |
| software | 软件 | "sell his software" |
| functionality | 功能 | "service, security, and functionality" |
| accepted | 接受 | "accept solutions proposed" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| cooking | "My cooking skills" |
| company | "business ideas" |
| team | "split teams" |
| culture | "other cultures" |
| law | "change the law" |

## 一句话总结

不要让“我想出来的”自动变成“最好的”；让不同角色评价不同方案，再用时间检验最初的自豪是否经得起比较。
