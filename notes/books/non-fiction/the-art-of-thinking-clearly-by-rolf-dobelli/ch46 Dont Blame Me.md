---
状态: 未读
modified: "2026-09-25"
source_text: ch46_45_don_t_blame_me_self_serving_bias.txt
---
# 46. Dont Blame Me（别怪我）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 45
- **作者**：Rolf Dobelli
- **章节定位**：关于自利性偏差
- **字符数**：4,189
- **一句话主旨**：人们把成功归因于自身能力，把失败归因于外部环境，必要时还会改写记忆来保护自我形象。

## 论证结构

- **核心论点**：自利性偏差是普遍的自我归因倾向，会污染学习、绩效评价和风险判断。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| CEO 年报 | 案例 | 好消息归功自己，坏消息归咎市场 |
| 成绩和投资收益 | 日常案例 | 成功自我归因，失败外部归因 |
| 随机分配人格测试分数 | 实验 | 高分者觉得测试公平，低分者觉得无用 |
| SAT 分数回忆 | 研究案例 | 人们会无意识地“增强”旧成绩 |
| 合租学生倒垃圾 | 日常调查 | 每个人都高估自己的贡献 |
| Lehman Brothers | 商业案例 | 自利性归因可能掩盖灾难责任 |

- **论证脉络**：CEO 年报 → 成绩与投资收益 → 随机实验机制 → 记忆高估 → 家庭贡献 → 邀请批评者。
- **可质疑处**：外部归因有时是合理的，统计相关和因果贡献需要结合反馈信息；自利性偏差的“偏误”定义不能替代对真实外部约束的分析。

## 选择性精读

### ① CEO 年报

> **原句 1:** "We attribute success to ourselves and failures to external factors."

**中文理解**：我们把成功归因于自己，把失败归因于外部因素。

**句子结构**：主语 We；谓语 attribute；两个 to 短语分别说明 success 与 failures 的归因方向。

**关键词**：attribute, success, failures, external factors

**表达方式**：用对称结构概括整章。

**为什么这样写**：它把复杂的事后叙事压缩成一个可识别的心理模式。

### ② 成绩归因

> **原句 2:** "If you got an A, you were solely responsible."

**中文理解**：如果你得了 A，你完全归功于自己。

**句子结构**：If 引导条件状语从句；主句 you were solely responsible。

**关键词**：got an A, solely responsible, grades

**表达方式**：用字母成绩作为高度压缩的成功符号。

**为什么这样写**：学校提供明确反馈，因此自利性归因可以被直接观察和类比迁移。

### ③ 随机分数实验

> **原句 3:** "Those who got scored highly found the test thorough and fair; low scorers rated it completely useless."

**中文理解**：得分高的人认为测试全面而公平；得分低的人则认为它完全没用。

**句子结构**：Those who got scored highly 为主语从句；found...fair 为动宾结构；low scorers 为第二个并列主语，rated...useless 为谓语。

**关键词**：scored highly, thorough and fair, low scorers, completely useless

**表达方式**：用同一测试的相反评价展示结果影响自我感受。

**为什么这样写**：随机分配分数排除了能力差异，评价反转显示人们会把结果解释成自我价值的证据。

### ④ 记忆增强

> **原句 5:** "They are neither lying nor exaggerating; they are simply “enhancing” the result a little."

**中文理解**：他们既没有撒谎，也没有夸大，只是稍微“美化”结果。

**句子结构**：neither...nor... 连接两个否定分句；simply emphasizing 为表语，quotes 标出隐含的修正意味。

**关键词**：lying, exaggerating, enhancing, result

**表达方式**：先排除明显欺骗，再给无意识改写命名。

**为什么这样写**：自利性偏差并不总是有意撒谎，而可能发生在诚实却有偏差的记忆中。

### ⑤ 倒垃圾比例

> **原句 6:** "Although their answers should have added up to 100 percent, these boys achieved an impressive 320 percent!"

**中文理解**：尽管答案加起来应该是 100%，这些男生却达到了惊人的 320%。

**句子结构**：Although 引导让步状语从句；主句 these boys achieved an impressive 320 percent。

**关键词**：answers, added up, 100 percent, 320 percent

**表达方式**：用明显超出总数的数字制造反讽。

**为什么这样写**：总和超过 100% 把个人贡献高估变成可触目可计算的事实。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| self-serving bias | 自利性偏差 | "Self-Serving Bias" |
| attribute | 归因于 | "We attribute success to ourselves" |
| external factor | 外部因素 | "failures to external factors." |
| enhancing | 美化 | "they are simply “enhancing” the result a little" |
| overestimated | 高估的 | "The five systematically overestimated their roles" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| annual report | 年报 | "read annual reports" |
| catalog | 列举 | "the CEO catalogs his indispensable contributions" |
| dynamic | 有活力的 | "a dynamic corporate culture" |
| random | 随机的 | "allocated the participants’ good or bad scores at random." |
| bankruptcy | 破产 | "until it went bankrupt" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| score | "scored highly" |
| failure | "failures to external factors" |
| test | "a personality test" |
| investment | "investment adviser" |
| blame | "the blame lies" |

## 一句话总结

自利性偏差最隐蔽之处在于它不必撒谎：只需把成功留给自己、把失败交给外部，就能让自我叙事继续感觉良好。
