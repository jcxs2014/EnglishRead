---
状态: 未读
modified: "2026-09-25"
source_text: ch98_97_the_stone_age_hunt_for_scapegoats_fal.txt
---
# 98. The Stone Age Hunt for Scapegoats（寻找替罪羊的古老冲动）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 97
- **作者**：Rolf Dobelli
- **章节定位**：关于单一原因谬误
- **字符数**：4,084
- **一句话主旨**：重大事件通常由无数因素共同造成，但人偏爱寻找一个原因、一个责任人和一个替罪羊。

## 论证结构

- **核心论点**：单一原因谬误把复杂因果压扁成单线故事；分析问题应列出影响因素、区分可改变因素，再用经验测试验证。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 战争动机访谈 | 媒体案例 | 人们持续追问唯一真实原因 |
| 2008 金融危机 | 历史案例 | 多个制度与行为因素共同作用 |
| 托尔斯泰苹果 | 文学类比 | 单一原因无法解释事件 |
| 早餐谷物产品经理 | 实务方法 | 用因素网络替代猜测 |
| 自由意志 | 哲学讨论 | 责任判断也受单一原因影响 |
| Tracy Chapman | 文化例子 | 商业叙事同样要求一个理由 |

- **论证脉络**：媒体追问 → 命名谬误 → 金融案例 → 文学类比 → 分析步骤 → 责任与权力。
- **可质疑处**：多因素分析容易变成事后故事；关键仍要靠实验、数据和时间顺序区分因素。

## 选择性精读

### ① 真实原因

> **原句 1:** "The real reason, not the sales pitch."

**中文理解**：我要的是真正原因，不是推销说辞。

**句子结构**：省略祈使句；The real reason 为强调宾语；not the sales pitch 为对比。

**关键词：** real reason, sales pitch, motive, war

**表达方式**：用简短对比逼问真正动机。

**为什么这样写**：媒体的单一原因问题会预设“存在一个隐藏在官方说法背后的原因”，这正是本章要拆解的直觉。

### ② 命名

> **原句 2:** "the fallacy of the single cause"

**中文理解**：单一原因谬误。

**句子结构**：名词短语作 the awkward phrase 的同位解释。

**关键词：** fallacy, single cause, awkward phrase, mental error

**表达方式**：用学术名称替代日常缺失的说法。

**为什么这样写**：命名让分散的新闻、金融和哲学案例归于同一认知错误。

### ③ 共同作用

> **原句 3:** "Not a single one, and yet every one of these, is the cause."

**中文理解**：没有哪一个单独的原因，但这些因素每一个都是原因。

**句子结构**：Not a single one 与 and yet every one... 形成对照。

**关键词：** not a single one, every one, the cause, factors

**表达方式**：用否定和肯定并置打破“单一原因”前提。

**为什么这样写**：复杂事件不是没有原因，而是由多个部分共同构成原因。

### ④ 网络

> **原句 4:** "After a while, you will have a network of possible influencing factors."

**中文理解**：过一段时间，你会得到一张可能影响因素的网络。

**句子结构**：After a while 为时间状语；you 为主语；will have 为谓语；a network 为宾语。

**关键词：** network, possible, influencing factors, empirical tests

**表达方式**：用“网络”取代单条因果链。

**为什么这样写**：网络允许因素互相影响，也提醒分析者不要过早锁定单一路径。

### ⑤ 替罪羊

> **原句 5:** "The idiotic hunt for a scapegoat goes hand in hand with the exercise of power."

**中文理解**：寻找替罪羊的愚蠢狩猎与行使权力手挽手并行。

**句子结构**：The idiotic hunt 为主语；goes hand in hand with... 为系表结构。

**关键词：** scapegoat, hunt, exercise of power, single reason

**表达方式**：用固定搭配把认知错误和权力结构相连。

**为什么这样写**：它把“为什么发生”追问提升为“谁来负责”的政治问题。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| fallacy of the single cause | 单一原因谬误 | "the fallacy of the single cause" |
| scapegoat | 替罪羊 | "hunt for a scapegoat" |
| causation | 因果关系 | "the complexity of causation" |
| factor | 因素 | "hundreds of factors" |
| free will | 自由意志 | "The notion of free will" |

### ⭐⭐ 进阶

| 词/phrase | 释义 | 例句（原文） |
|---|---|---|
| motive | 动机 | "the motive behind" |
| corruption | 腐败 | "Corrupt auditors" |
| crisis | 危机 | "the financial crisis" |
| influence | 影响 | "influencing factors" |
| empirical | 经验的 | "empirical tests" |

### ⭐ 基础

| 词/phrase | 例句（原文） |
|---|---|
| war | "the war" |
| bank | "Banks caved" |
| investor | "Investors" |
| reason | "The real reason" |
| test | "empirical tests" |

## 一句话总结

遇到大事不要急着问“谁造成”，先画出因素网络，再用数据测试哪些变量真正改变了结果。
