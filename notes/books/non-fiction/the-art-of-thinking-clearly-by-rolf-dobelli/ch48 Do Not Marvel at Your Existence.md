---
状态: 未读
modified: "2026-09-25"
source_text: ch48_47_do_not_marvel_at_your_existence_self_.txt
---
# 48. Do Not Marvel at Your Existence（不要惊叹自己的存在）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 47
- **作者**：Rolf Dobelli
- **章节定位**：关于自我选择偏误
- **字符数**：3,876
- **一句话主旨**：我们只注意自己经历中的坏运气、异常或成功，因为其他可能结果没有留下记忆和观察样本。

## 论证结构

- **核心论点**：自我选择偏误使人高估“偏偏发生在我身上”的特殊事件，却忽略大量未被观察的普通背景。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 堵车时只看见对向车流 | 日常观察 | 交通状态决定人如何感知时间比例 |
| 十个红绿灯 | 概率例 | 低概率状态占据更多感受时间 |
| 男女朋友对公司比例的抱怨 | 社会观察 | 抱怨者自身属于少数群体 |
| 通讯录调查 | 调查案例 | 只回应满意订户会造成样本偏差 |
| 每户至少一部电话的调查结果 | 调查讽刺 | 没电话者没有机会进入样本 |

- **论证脉络**：堵车体验 → 时间暴露偏差 → 群体归属与投票 → 调查样本 → 哲学家的生存惊叹 → 电话调查反讽。
- **可质疑处**：样本选择确实会扭曲统计，但个人实际经历的坏事也可能真实且重要；不能把“未观察到”都解释成随机或无意义。

## 选择性精读

### ① 堵车体验

> **原句 1:** "As I spent the next hour crawling forward at a snail’s pace, and started to grow restless from braking and accelerating, I asked myself whether I really was especially unlucky."

**中文理解**：接下来一个小时我像蜗牛一样缓慢前进，并因反复刹车和加速而烦躁，于是问自己是否真的特别倒霉。

**句子结构**：As 引导时间/原因状语从句；主句 I asked myself...；whether 引导宾语从句。

**关键词：** crawling forward, snail’s pace, braking and accelerating, especially unlucky

**表达方式**：用身体速度比喻强化不耐烦感。

**为什么这样写**：堵车让时间膨胀，读者能感到“似乎总轮到红灯”的体验从何而来。

### ② 概率与时间比例> **原句 2:** "The probability that I will get stuck in a jam on a particular day is not greater than the probability that one will occur."

**中文理解**：某一天我陷入堵车的概率不会高于发生堵车的概率。

**句子结构**：主语从句 The probability... 后接 not greater than 比较结构；两个 probability 形成并列。

**关键词：** probability, get stuck, particular day, not greater than

**表达方式**：用重复 probability 区分事件概率与某时点暴露。

**为什么这样写**：数学上不把某一天放大，但停滞状态持续时间更长，感知就会产生偏差。

### ③ 红灯时间> **原句 3:** "On average, one out of the ten will always be red, and the others green."

**中文理解**：平均而言，十个红灯中总有一个是红灯，其他都是绿灯。

**句子结构**：one out of the ten 作主语；will always be red 为谓语；and the others green 为省略结构。

**关键词：** On average, one out of the ten, red, green

**表达方式**：用十次循环的小模型把概率视觉化。

**为什么这样写**：读者可以自己检查平均值与等待感受之间的差异。

### ④ 样本选择> **原句 4:** "The grumblers form part of the sample."

**中文理解**：抱怨者本身就构成了样本的一部分。

**句子结构**：The grumblers 为主语；form part of the sample 为系表结构。

**关键词：** grumblers, form part, sample

**表达方式**：用简短事实句揭示样本构成。

**为什么这样写**：如果只观察抱怨者，就会把少数体验误认为整体规律。

### ⑤ 通讯录调查> **原句 5:** "Unfortunately, this reaches only one group: current subscribers who are clearly satisfied, have time to respond, and have not canceled their subscriptions."

**中文理解**：遗憾的是，调查只接触到一个群体：目前满意、有时间回复且没有取消订阅的订户。

**句子结构**：this 为主语；reaches only one group 为谓语；冒号后的现在分词短语解释群体特征。

**关键词：** reaches only one group, current subscribers, canceled, subscriptions

**表达方式**：用三项筛选条件把样本偏差具体化。

**为什么这样写**：调查并非直接测量满意度，而是先测量“愿意继续留下的人”。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| self-selection bias | 自我选择偏误 | "Self-Selection Bias" |
| sample | 样本 | "form part of the sample" |
| probability | 概率 | "The probability that I will get stuck..." |
| questionnaire | 问卷 | "send out a questionnaire." |
| nonexistence | 不存在 | "their nonexistence" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| traffic jam | 堵车 | "a traffic jam develops" |
| disproportionate | 比例失衡的 | "a disproportionate amount of time" |
| grumble | 抱怨 | "Whenever we complain" |
| subscriber | 订户 | "current subscribers" |
| survey | 调查 | "this recent telephone survey" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| average | "On average" |
| light | "traffic lights" |
| phone | "how many phones" |
| household | "each household owned" |
| company | "how much customers value their newsletter" |

## 一句话总结

你更容易记住并看见发生在自己身上的坏结果；判断运气前，要先问样本如何形成、哪些结果根本没有机会被你观察到。