---
状态: 未读
modified: "2026-09-25"
source_text: ch70_69_disregard_the_brand_new_neomania.txt
---
# 70. Disregard the Brand New（别迷信全新的东西）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 69
- **作者**：Rolf Dobelli
- **章节定位**：关于新事物迷恋
- **字符数**：3,582
- **一句话主旨**：预测未来时应重视经受过时间检验的旧技术，不要把短期新奇和显眼发明误认为长期进步。

## 论证结构

- **核心论点**：新技术的寿命通常短于传统技术；未来预测需要以历史筛选出的持久方案为基准，而非追逐流行趋势。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 五十年前的未来想象 | 历史类比 | 许多当年预测并未实现 |
| 椅子、裤子、鞋、书架、叉子 | 传统技术 | 旧技术具有长期稳定性 |
| Taleb 的反脆弱观点 | 理论论断 | 经受时代检验者更可能延续 |
| 早期采用者 | 社会观察 | 新奇性取代了实际收益 |
| 《2001：太空漫游》 | 影视预测 | 未来想象容易投射时代局限 |

- **论证脉络**：未来想象 → 反观日常物品 → 反脆弱原则 → 新迷恋 → 电影反例 → 历史过滤器。
- **可质疑处**：旧技术也可能在功能、效率或安全性上被新方案淘汰；“存活越久越可靠”不能替代具体领域的证据评估。

## 选择性精读

### ① 未来想象

> **原句 1:** "We would live in plastic capsules, work in underwater cities, vacation on the moon."

**中文理解**：我们会住在塑料胶囊里，在水下城市工作，到月球度假。

**句子结构**：三个 would 引导的并列动词短语；live、work、vacation 共用 would。

**关键词**：plastic capsules, underwater cities, moon, future

**表达方式**：用三个平行未来场景制造想象冲击。

**为什么这样写**：并列让当时的技术想象显得完整、必然，也为后文的反差提供落点。

### ② 旧物

> **原句 2:** "Nothing has changed."

**中文理解**：什么都没有改变。

**句子结构**：Nothing 为不定代词；has changed 为现在完成时系表结构。

**关键词**：Nothing, changed, invention, future

**表达方式**：用极短否定句总结前面长串例子。

**为什么这样写**：当读者刚看见未来预言时，“什么都没有改变”立即制造认知反转。

### ③ 反脆弱

> **原句 3:** "Old technology has proven itself; it possesses an inherent logic even if we do not always understand it."

**中文理解**：旧技术已经证明了自己；即便我们不完全理解，它也具有内在逻辑。

**句子结构**：第一个分句为现在完成时；第二个分句为现在时，even if 引导让步状语从句。

**关键词**：Old technology, proven itself, inherent logic, understand

**表达方式**：用分号连接经验结论与解释性补充。

**为什么这样写**：它把“古老”从怀旧偏好改写成经过时间检验的证据。

### ④ 新迷恋

> **原句 4:** "To them, it is of minor importance if an invention provides tangible benefits; novelty matters more."

**中文理解**：对他们来说，发明是否带来实际好处并不重要；新奇性更重要。

**句子结构**：if 引导条件状语从句；分号连接两个对比判断。

**关键词**：minor importance, tangible benefits, novelty

**表达方式**：用分号突出“实际收益”和“新奇感”的价值倒置。

**为什么这样写**：它批评的不是购买新事物本身，而是把新当作替代效果标准的判断方式。

### ⑤ 存活法则

> **原句 5:** "Whatever has survived for X years will last another X years."

**中文理解**：任何存活了 X 年的东西，还会再延续 X 年。

**句子结构**：Whatever 引导主语从句；has survived 与 will last 为两个谓语。

**关键词**：Whatever, survived, X years, last another X years

**表达方式**：用对称的 X 年结构把经验压缩成可记忆规则。

**为什么这样写**：它不是证明，而是一个降低未来预测过度自信的粗略筛选启发式。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| neomania | 新事物迷恋 | "neomania pitfall" |
| antifragile | 反脆弱的 | "Antifragile" |
| inherent | 内在的 | "inherent logic" |
| tangible | 实际的；可触摸的 | "tangible benefits" |
| held | 保持住 | "Whatever has held its own" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| capsule | 胶囊；舱体 | "plastic capsules" |
| contraption | 装置 | "magic contraptions" |
| early adopter | 早期采用者 | "so-called early adopters" |
| gimmick | 花招；噱头 | "sort the gimmicks" |
| enduring | 持久的 | "If something has endured" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| future | "the future" |
| city | "cities" |
| shoes | "your leather shoes" |
| invention | "an invention" |
| strategy | "a strategy meeting" |

## 一句话总结

预测未来时，先问什么已经被时间保留下来；新事物可以带来惊喜，但新奇本身不是价值证据。
