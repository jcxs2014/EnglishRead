---
状态: 未读
modified: "2026-09-25"
source_text: ch03_2_does_harvard_make_you_smarter_swimmer_.txt
---
# 03. Does Harvard Make You Smarter?（哈佛让你更聪明吗？）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 2
- **作者**：Rolf Dobelli
- **章节定位**：全书第 2 个认知偏差单元
- **字符数**：约 3,600 字符
- **一句话主旨**：作者用职业运动员、模特和名校学生说明，优秀结果常来自先天或筛选条件，而非被误认为结果本身的条件；这就是“游泳运动员身体幻觉”。

## 论证结构

- **核心论点**：人们常把进入某个结果群体之前的筛选因素误当成导致该结果的因素，于是把选择效应误读为教育、外貌或能力的效果。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|------|------|---------|
| 游泳运动员身材与训练关系倒置 | 案例 | 身体条件是入选因素，不是训练结果 |
| 模特与化妆品广告 | 类比 | 被展示的外貌不是产品制造的结果 |
| 哈佛学生成功与学校质量的混淆 | 案例 | 名校可能主要筛选优秀学生 |
| MBA 毕业生收入比较 | 研究解释 | 选择与结果之间还受其他背景因素影响 |

- **论证脉络**：运动员故事 → 模特的反向例子 → 推广到名校 → 解释 MBA 收入统计 → 延伸到幸福感 → 建议在投入前检查筛选条件。
- **可质疑处**：作者用个人对商学院的旧体验作论据，证据强度有限；“学校可能糟糕但学生优秀”是可能性陈述，不足以证明所有教育机构都没有教育效果。

## 选择性精读

### ① 因果倒置

> **原句 1:** "Professional swimmers don’t have perfect bodies because they train extensively. Rather, they are good swimmers because of their physiques."

**中文理解**：职业游泳运动员并不是因为大量训练才拥有完美身材；恰恰是因为他们拥有适合游泳的身体，他们才是优秀的游泳者。

**句子结构**：两个分句以 not... because... 和 Rather, ... because... 构成明确因果反转。

**关键词：** **：professional swimmers, perfect bodies, train extensively, physique

**表达方式**：用重复的 because 形成同词反义，强化因果关系的倒置。

**为什么这样写**：先把大众熟悉的“训练塑造身体”翻转成“身体帮助成为运动员”，让选择效应变得可见。

### ② 选择因素
> **原句 2:** "Whenever we confuse selection factors with results, we fall prey to what Taleb calls the swimmer’s body illusion."

**中文理解**：每当我们混淆选择因素与结果，就会落入 Taleb 所谓的“游泳运动员身体幻觉”。

**句子结构**：Whenever 引导时间/条件从句；主句为 we fall prey to what Taleb calls the swimmer’s body illusion。

**关键词：** **：selection factors, results, fall prey to, swimmer’s body illusion

**表达方式**：用 Whenever 把错误普遍化，再用专名把现象固定成可记忆标签。

**为什么这样写**：作者先抽象命名偏差，再让读者把运动员、模特和名校案例归入同一框架。

### ③ 模特的反向例子
> **原句 3:** "The models are born attractive, and only for this reason are they candidates for cosmetics advertising."

**中文理解**：模特天生具有吸引力，也正因为如此，她们才成为化妆品广告的候选人。

**句子结构**：The models 为主语；are born attractive 为一般现在时谓语；only for this reason 置于句首引起强调，are candidates... 使用部分倒装。

**关键词：** **：born attractive, candidates, cosmetics advertising

**表达方式**：把“产品制造美”的广告逻辑改写成“外貌带来广告资格”的选择逻辑。

**为什么这样写**：同一个身体案例被迁移到商业广告，说明偏差并不限于体育领域。

### ④ 哈佛问题
> **原句 4:** "Many highly successful people have studied there. Does this mean that Harvard is a good school? We don’t know."

**中文理解**：许多非常成功的人曾在哈佛学习。这是否意味着哈佛是一所好学校？我们不知道。

**句子结构**：前句是事实陈述；后两句构成反问与直接回答，We don’t know 刻意限制推论强度。

**关键词：** **：highly successful, studied there, good school

**表达方式**：先呈现相关性，再用问句和不确定回答阻止读者立即得出因果结论。

**为什么这样写**：名校的声望容易制造教育效果幻觉；作者故意不替哈佛下结论，而要求读者保持证据克制。

### ⑤ MBA 统计
> **原句 5:** "Many prospective students fall for this approach."

**中文理解**：许多 prospective students 会被这种方法吸引。

**句子结构**：主语 Many prospective students + 谓语 fall for + 宾语 this approach。

**关键词：** **：prospective students, fall for, approach

**表达方式**：用 fall for 的日常搭配表现“被统计话术说服”的轻率与自我暴露。

**为什么这样写**：作者把批判对象具体化为“准学生如何接受 MBA 收入承诺”，避免只停留在抽象的选择偏差。

### ⑥ 幸感的错觉
> **原句 6:** "They do not realize that cheerfulness—according to many studies, such as those conducted by Harvard’s Dan Gilbert—is largely a personality trait that remains constant throughout life."

**中文理解**：他们没有意识到，愉快在许多研究看来主要是一种贯穿一生的性格特征，例如哈佛 Dan Gilbert 的研究就持这种看法。

**句子结构**：主句为 They do not realize；that 宾语从句内含 according to 插入语，is largely a personality trait... 为系表结构，that 引导定语从句修饰 trait。

**关键词：** **：cheerfulness, personality trait, remains constant

**表达方式**：用插入的研究来源压低断言的权威性，再用 remains constant 把“训练 happiness”的想象固定化。

**为什么这样写**：作者把偏差从学校和收入扩展到自我成长，提示读者不要把所有结果都理解为可由努力制造。

## 词汇分级

### ⭐⭐⭐ 高级
| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| physique | 体格；身材 | "they are good swimmers because of their physiques." |
| selection factor | 选择因素 | "Whenever we confuse selection factors with results..." |
| fall prey to | 成为……的猎物；受……支配 | "we fall prey to what Taleb calls the swimmer’s body illusion." |
| prospective students | 准学生 | "Many prospective students fall for this approach." |
| personality trait | 性格特征 | "They do not realize that cheerfulness—according to many studies, such as those conducted by Harvard’s Dan Gilbert—is largely a personality trait that remains constant throughout life." |

### ⭐⭐ 进阶
| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| cosmetics advertising | 化妆品广告 | "only for this reason are they candidates for cosmetics advertising." |
| income gap | 收入差距 | "The income gap between both groups stems from a multitude of reasons." |
| factor for selection | 用于选择的因素 | "How their bodies are designed is a factor for selection and not the result of their activities." |
| rigorously | 严格地 | "Most probable, however, is the rigorous selection." |
| self-illusion | 自我幻觉 | "The swimmer’s body illusion is also a self-illusion." |

### ⭐ 基础
| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| model | 模特 | "The models are born attractive..." |
| school | 学校 | "Does this mean that Harvard is a good school?" |
| reason | 原因 | "The reason behind this is unknown." |
| person | 人 | "It is as if these individuals do not realize..." |
| happy | 快乐的 | "When I ask happy people about the secret of their contentment..." |

## 一句话总结

游泳运动员、模特和名校学生的共同点不是“成功制造了优秀条件”，而是我们只看到了被条件筛选后的成功者；真正需要追问的，是谁在进入结果之前就已经拥有优势。