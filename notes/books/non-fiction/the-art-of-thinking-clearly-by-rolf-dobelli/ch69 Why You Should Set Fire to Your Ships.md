---
状态: 未读
modified: "2026-09-25"
source_text: ch69_68_why_you_should_set_fire_to_your_ships.txt
---
# 69. Why You Should Set Fire to Your Ships（为什么要烧掉自己的船）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 68
- **作者**：Rolf Dobelli
- **章节定位**：关于无法关闭选择
- **字符数**：4,050
- **一句话主旨**：保留所有选项会消耗时间、精力和成功率；有效策略必须明确写出不做什么。

## 论证结构

- **核心论点**：选择的机会成本常被隐藏，人们因而同时追逐过多可能；关闭选项能把注意力集中到真正重要的行动。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 二十四本书 | 个人例子 | 并读不产生深入洞见 |
| 同时与三位女性约会 | 关系类比 | 不选择就难有真实关系 |
| 项羽焚船、Cortés 沉船 | 历史案例 | 消除退路能集中注意力 |
| 电脑游戏 | 实验 | 保持所有门开放导致低效 |
| CEO 与销售案例 | 商业应用 | 全面选择通常等于没有战略 |

- **论证脉络**：个人困扰 → 历史策略 → 实验证据 → 机会成本 → 关门清单。
- **可质疑处**：不可逆决策需要信息，仓促关闭选项也会造成损失；作者主张的是预先界定边界，而非机械排除一切可能。

## 选择性精读

### ① 并读

> **原句 1:** "I know that sporadic reading won’t help me achieve any real insights."

**中文理解**：我知道，断断续续的阅读无法帮助我获得真正的洞见。

**句子结构**：I know 后接 that 从句；won’t help 为否定谓语；any real insights 为宾语。

**关键词**：sporadic reading, real insights, twenty-four

**表达方式**：以自我诊断开篇，承认问题首先是知识管理失败。

**为什么这样写**：作者不是泛泛劝人专注，而是把自己的二十四本书作为反例。

### ② 约会类比

> **原句 2:** "If he refrains from deciding, all options remain open."

**中文理解**：如果他拒绝决定，所有选项就仍然开放。

**句子结构**：If 引导条件状语从句；主句为 all options remain open。

**关键词**：refrains from deciding, all options, remain open

**表达方式**：用条件句呈现“不选择”的虚假自由。

**为什么这样写**：不选择看似避免损失，实际让每项关系都无法获得真实承诺。

### ③ 沉船

> **原句 3:** "By removing the option of retreat, he switched their focus to the only thing that mattered: the battle."

**中文理解**：通过取消撤退选择，他把他们的注意力转向唯一重要的事情：战斗。

**句子结构**：By 引导方式状语；he switched...；the only thing that mattered 作同位解释，冒号后 battle 作强调。

**关键词**：removing, option of retreat, focus, mattered

**表达方式**：用“撤退选项”具体化注意力分散的原因。

**为什么这样写**：沉船的戏剧性把战略选择讲成一次不可逆的注意力集中。

### ④ 游戏结果

> **原句 4:** "Sacrificing options was a price they were not willing to pay."

**中文理解**：牺牲选项是他们不愿支付的价格。

**句子结构**：Sacrificing options 为主语；was 为系动词；a price... 为表语。

**关键词**：Sacrificing options, a price, not willing to pay

**表达方式**：把抽象选择写成一张心理账单。

**为什么这样写**：它揭示人们愿意承受效率损失，只因为放弃可能性的痛苦即时而可见。

### ⑤ 不做什么

> **原句 5:** "A business strategy is primarily a statement on what not to engage in."

**中文理解**：商业战略首先是一份关于不参与什么的声明。

**句子结构**：A business strategy 为主语；is 为系动词；a statement on... 为表语。

**关键词**：business strategy, statement, not to engage in

**表达方式**：用否定式定义战略，扭转“战略意味着选择更多”的直觉。

**为什么这样写**：战略的价值不在于列出愿望，而在于排除会分散资源的方向。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| option | 选择；期权 | "The downside" |
| retreat | 撤退 | "the option of retreat" |
| unproductive | 无成效的 | "All this unproductive scrambling" |
| intangible | 无形的 | "the price tag is often hidden and intangible" |
| disregard | 排除；忽视 | "Disregard certain possibilities" |

### ⭐⭐ 进阶

| 词/phrase | 释义 | 例句（原文） |
|---|---|---|
| sporadic | 断断续续的 | "sporadic reading" |
| doors | 门 | "new door cracks open" |
| anxiety | 焦虑 | "The same anxiety kicked in" |
| consumer | 消费者 | "all customer segments" |
| options | 选择 | "all potential treasure troves" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| book | "twenty-four books" |
| door | "keep all doors open" |
| strategy | "A business strategy" |
| sales | "Salespeople" |
| focus | "switched their focus" |

## 一句话总结

选择不是把所有门留着，而是先决定哪些门永远不打开；战略的锋利处，正体现在它明确拒绝什么。
