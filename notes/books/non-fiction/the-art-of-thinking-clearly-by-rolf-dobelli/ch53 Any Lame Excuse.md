---
状态: 未读
modified: "2026-09-25"
source_text: ch53_52_any_lame_excuse_because_justification.txt
---
# 53. Any Lame Excuse（任何借口）

## 概览

- **出处**：*The Art of Thinking Clearly*，Chapter 52
- **作者**：Rolf Dobelli
- **章节定位**：关于“因为”式辩解
- **字符数**：4,435
- **一句话主旨**：人们并不总要求理由充分，只要给出一个带有 because 的理由，就更容易获得容忍、合作与安心。

## 论证结构

- **核心论点**：理由的价值不仅在于内容真假，也在于它为行为提供了可理解的叙事；“because”能把任意行动包装成有目的的行为。
- **证据链**：

| 证据 | 类型 | 支撑什么 |
|---|---|---|
| 高速公路施工 | 日常案例 | 明确理由降低堵车带来的挫折 |
| Ellen Langer 复印实验 | 实验研究 | 给出理由后，插队成功率大幅提高 |
| 航班延误公告 | 日常案例 | “operational reasons” 即使空泛也能安抚乘客 |
| 企业目的宣言 | 商业语言 | 高尚故事能动员员工 |
| 股市评论 | 案例 | 人们偏好可触摸的理由，而非噪音或复杂性 |

- **论证脉络**：堵车经历 → 复印实验 → because 的心理作用 → 组织、商业和媒体语言 → 家庭小事 → 结论。
- **可质疑处**：理由有时能提供真实信息；如果把所有理由都归为借口，会忽视合理说明、协作沟通和责任追溯。

## 选择性精读

### ① 复印实验

> **原句 1:** "Her success rate was 60 percent."

**中文理解**：她的成功率为 60%。

**句子结构**：Her success rate 为主语；was 为系动词；60 percent 为表语。

**关键词：** **：success rate, 60 percent

**表达方式**：用精确百分比建立实验可信度。

**为什么这样写**：百分比让“理由是否有用”的差异从模糊感受变成可比较数据。

### ② 加入理由
> **原句 2:** "In almost all cases (94 percent), she was allowed to go ahead."

**中文理解**：在几乎所有情况下（94%），她都获准先使用。

**句子结构**：In almost all cases 为状语；括号补充精确比例；主句 she was allowed to go ahead 为被动结构。

**关键词：** **：almost all cases, 94 percent, allowed, go ahead

**表达方式**：用 almost 与精确百分比并置。

**为什么这样写**：理由的效果并不只是“更友好”，而是大幅改变陌生人的合作判断。

### ③ 理由的充分性
> **原句 3:** "It seems to matter very little if your excuse is good or not."

**中文理解**：你的借口好不好，似乎并不那么重要。

**句子结构**：It 为形式主语；to matter very little 为真正主谓；if 引导让步状语从句。

**关键词：** **：seems, matter very little, excuse, good or not

**表达方式**：用保留语 seems 把观察与普遍定律区分开。

**为什么这样写**：它提出核心判断：社会互动首先需要理由的格式，而不总是理由的质量。

### ④ 航班公告
> **原句 4:** "Flight 5678 is delayed by three hours due to operational reasons."

**中文理解**：5678 航班因运营原因延误三小时。

**句子结构**：Flight 5678 为主语；is delayed 为被动谓语；due to operational reasons 为原因状语。

**关键词：** **：Flight, delayed, due to, operational reasons

**表达方式**：用正式公告和空泛原因制造秩序感。

**为什么这样写**：乘客无法从“运营原因”获得可行动信息，却因解释存在而感到被告知。

### ⑤ 鞋业公司的目的
> **原句 5:** "We want our shoes to revolutionize the market."

**中文理解**：我们希望让鞋子革新市场。

**句子结构**：We 为主语；want 为谓语；our shoes 为宾语；to revolutionize the market 为不定式补语。

**关键词：** **：revolutionize, market

**表达方式**：用宏大目的替代“制造鞋子”的平淡功能。

**为什么这样写**：高层目的需要被叙述成改变世界的使命，才能动员组织并为行动赋予意义。

## 词汇分级

### ⭐⭐⭐ 高级

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| justification | 辩解；理由 | "“Because” Justification" |
| pretext | 借口 | "the pretext was (a-hem) paper-thin" |
| redundant | 多余的 | "completely redundant" |
| rally | 召集；鼓舞 | "provide no rallying call" |
| revolutionize | 革新 | "revolutionize the market" |

### ⭐⭐ 进阶

| 词/短语 | 释义 | 例句（原文） |
|---|---|---|
| photocopier | 复印机 | "waited at a photocopier" |
| tolerate | 容忍 | "encounter more tolerance" |
| operational | 运营的 | "operational reasons" |
| morale | 士气 | "employee motivation" |
| commentator | 评论员 | "stock market commentators" |

### ⭐ 基础

| 词/短语 | 例句（原文） |
|---|---|
| because | "because I’m in a rush?" |
| line | "a line had formed" |
| market | "the stock market rises or falls" |
| purpose | "the purpose of your shoe company" |
| reason | "for any reason" |

## 一句话总结

理由不必总 elaborate 才能让人接受，但一个真正的理由应当经得起追问；否则它只是让行为听起来像有意图。