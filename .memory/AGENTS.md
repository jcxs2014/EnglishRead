...
### Ripeness（Sarah Moss）双线文学小说独立审查整改（d014e7b3）
- **结构**：双线（73 岁 Edith 当下 County Clare + 17 岁 Edith 意大利陪产 Lydia），精简格式（导航 5 项 + 8 处四子项精读 + 三档词汇 + 一句话总结），保留总览三篇（AGENTS 文学小说"保留三篇总览"惯例）
- **体裁定级**：文学小说（非悬疑/推理/言情/非虚构），精简格式适用；但总览三篇按 Lonely Mouth / New Skin 先例保留（用户验收时未要求删除）
- **审查抓出的总览层事实缺陷**（AGENTS 第 10 条 e 款"总览层事实核对"执行）：
  1. 概述人物弧光"'你'（Gabriel/Pat 之谜）"误标——Pat 是 Edith 亲儿子（ch01 "after Pat was born"），"你"的表哥（ch24 "assure Patrick – your cousin"），非"你"。改"Gabriel，Lydia 之子；Pat 的表哥"
  2. 概述"被 Igor 与 Emil 轮奸怀孕"措辞过重——ch12 原文 "you're the child of rape, although at the time it wasn't, exactly"（醉酒不记得、衣服撕裂、有血迹），非明确 gang rape。改"醉酒后与二人发生关系"
  3. 概述/情感节点"照料五天"无原文支撑——ch32 Signora Pilone 明言 "you have crossed each other's paths for a week"，ch26 Edith 自数 "three nights, two days"。统一改"约一周"
- **新增教训**：
  - 概述中所有"人物身份/关系/结局/天数"陈述须对原文 grep 验证（"轮奸"是凭印象写作的典型案例）
  - 精简格式总览三篇按惯例保留而非删除（Lonely Mouth 先例已确立）
  - verify_overview_quotes 工具不覆盖概述行内英文片段，须自备 flat grep 全量兜底
  - 概述里"你=Pat 之谜"这类标题本身是缺陷信号——Pat 是明确人物，"之谜"暗示混淆即错
- **方法论固化**：总览层缺陷模式——凭印象写作（"轮奸"）、跨人物身份混淆（Pat/Gabriel）、无原文数字天数（"五天"），均需独立 grep 验证，不能仅依赖工具口径
- **独立审查 SOP 五步法实战验证**：a 三件套重跑+复验 / b 逐章归属 / c 结构扫描 / d 语义二审（子代理额度耗尽主会话自执行）/ e 总览事实核对——本次 e 步骤抓出 3 处整改证实该步骤在工具全绿时仍有价值（与 Room 37% 说话人误归同族教训）

## 2026-09-12 新增

### 多批次独立五步审查实战汇总
- 2026-09-12 全天完成 Payback Plan / What If It's You / Ripeness 三本书的独立五步审查，证实 SOP 第 10 条"独立审查"流程可独立操作（执行方即审查方场景下，必须重跑三件套不复信执行数字）
- 全库 24 书批次审查实战教训：
  - 子代理附防幻觉条款（"报警前必须确认引语行与中文理解行真实存在且相邻"）+ 1-2 个本库真实失败案例可显著降低误报率（New Skin 43 处整改中 1 处子代理幻觉被驳回即由此）
  - 子代理额度耗尽时主会话自执行不可省（What If It's You / Ripeness 均触发此情形）
  - **总览层 e 步骤在工具全绿时仍有抓出缺陷的能力**（Room 30 条金句 11 处说话人误归 / Payback Plan 0 误归但通过其他路径查核 / Ripeness 概述 3 处事实缺陷）
  - **跨书污染自检**：任何不熟悉的人名/地名先 `grep -rl "<name>" notes/books/` 排除（Payback Plan 验证无外溢）

### 已完成精读批次（2026-09-12）
- **The Payback Plan（Amy Andrews）**：20 单元 + 总览三篇，10 commits，言情长篇逐章格式。门禁 verify 180/180 / vocab 549 词条 FAIL=0 WARN=0 / entities 0 / chapter 157/157 / overview 23/23 / crossref 0
- **What If It's You（Jilly Gagnon）**：22 单元 + 总览三篇，12 commits，言情长篇+平行现实设定。门禁 verify 200/200 / vocab 564 词条 FAIL=0 WARN=0 / entities 0 / chapter 175/175 / overview 25/25 + 人工兜底 26/26 / crossref 0 / 跨书污染 0
- **Ripeness（Sarah Moss）**：34 单元 + 总览三篇，15 commits，文学小说双线精简格式。门禁 verify 268/268 / vocab 483 词条 FAIL=0 WARN=6 / entities 0 / chapter 268/268 / overview 52/52 / crossref 0 / 关键词锚定 0 / 结构 273 块零缺陷 / 独立审查抓出 3 处总览层事实缺陷整改
